...
class TypeScriptAstTransformer {
    ...
    requestNewMemberInObjectLiteral(visitCondition, propertyNameOrAssignment, propertyValue, multiline) {
        let newProperty;
        ...
        const transformerFactory = (0, TransformerFactories_1.newMemberInObjectLiteralTransformerFactory)(newProperty, visitCondition, multiline, this._expressionCollector);
     ...
     */
    requestUpdateForObjectLiteralMember(visitCondition, targetMember) {
        const transformerFactory = (0, TransformerFactories_1.updateForObjectLiteralMemberTransformerFactory)(visitCondition, targetMember);
        this.requestChange(types_1.ChangeType.NodeUpdate, transformerFactory, types_1.SyntaxKind.PropertyAssignment, ts.factory.createPropertyAssignment(targetMember.name, targetMember.value));
    }
    requestNewMembersInArrayLiteral(visitCondition, expressionOrPropertyAssignment, prepend = false, anchorElement, multiline = false) {
        let elements;
        ...
        if (isExpression) {
            ...
            elements = expressionOrPropertyAssignment.map((property) => this._factory.createObjectLiteralExpression([property], multiline));
        }
        const transformerFactory = (0, TransformerFactories_1.newMemberInArrayLiteralTransformerFactory)(visitCondition, elements, prepend, anchorElement);
