...
exports.newImportDeclarationTransformerFactory = exports.newArgumentInMethodCallExpressionTransformerFactory = exports.newMemberInArrayLiteralTransformerFactory = exports.updateForObjectLiteralMemberTransformerFactory = exports.newMemberInObjectLiteralTransformerFactory = void 0;
exports.createImportDeclaration = createImportDeclaration;
exports.importDeclarationCollides = importDeclarationCollides;
 ...
 */
const newMemberInObjectLiteralTransformerFactory = (newProperty, visitCondition, multiline, expressionCollector) => {
    return (context) => {
        return (rootNode) => {
            const visitor = (node) => {
                if (ts.isObjectLiteralExpression(node) && visitCondition(node)) {
                    const existingProperty = node.properties.find((property) => ts.isPropertyAssignment(property) &&
                    ...
                    if (ts.isArrayLiteralExpression(existingPropInitializer)) {
                        return updatePropertyAssignmentWithArrayLiteral(newProperty, existingProperty, context, node, expressionCollector, multiline);
                    }
                    return updatePropertyAssignmentWithIdentifier(newProperty, existingProperty, context, node);
                }
                return ts.visitEachChild(node, visitor, context);
            };
...
exports.newMemberInObjectLiteralTransformerFactory = newMemberInObjectLiteralTransformerFactory;
/**
 * Creates a {@link ts.TransformerFactory} that updates a member in a {@link ts.ObjectLiteralExpression}.
 */
const updateForObjectLiteralMemberTransformerFactory = (visitCondition, targetMember) => {
    return (context) => {
        return (rootNode) => {
            const visitor = (node) => {
                if (ts.isObjectLiteralExpression(node) && visitCondition(node)) {
                    const newProperties = node.properties.map((property) => {
                        ...
                        if (isPropertyAssignment &&
                            ...
                            return context.factory.updatePropertyAssignment(property, property.name, targetMember.value);
                        }
                        return property;
                    });
                    return context.factory.updateObjectLiteralExpression(node, newProperties);
                }
                return ts.visitEachChild(node, visitor, context);
            };
            return ts.visitNode(rootNode, visitor, ts.isSourceFile);
...
exports.updateForObjectLiteralMemberTransformerFactory = updateForObjectLiteralMemberTransformerFactory;
/**
 * Creates a {@link ts.TransformerFactory} that adds a new element to a {@link ts.ArrayLiteralExpression}.
 */
const newMemberInArrayLiteralTransformerFactory = (visitCondition, elements, prepend = false, anchorElement) => {
    return (context) => {
        return (rootNode) => {
            const visitor = (node) => {
                let anchor;
                if (ts.isArrayLiteralExpression(node) && visitCondition(node)) {
                    ...
                    if (anchor) {
                        ...
                        return context.factory.updateArrayLiteralExpression(node, structure);
                    }
                    if (prepend) {
...
exports.newMemberInArrayLiteralTransformerFactory = newMemberInArrayLiteralTransformerFactory;
/**
 * Creates a {@link ts.TransformerFactory} that adds a new argument to a {@link ts.CallExpression}.
 */
const newArgumentInMethodCallExpressionTransformerFactory = (visitCondition, argument, position = -1, override = false) => {
    return (context) => {
        return (rootNode) => {
            const visitor = (node) => {
                if (ts.isCallExpression(node) && visitCondition(node)) {
                    ...
                    if (position >= 0) {
                        ...
                        return ts.factory.updateCallExpression(node, node.expression, node.typeArguments, updatedArguments);
                    }
...
exports.newArgumentInMethodCallExpressionTransformerFactory = newArgumentInMethodCallExpressionTransformerFactory;
/**
 * Creates a {@link ts.TransformerFactory} that adds a new {@link ts.ImportDeclaration}.
 */
const newImportDeclarationTransformerFactory = (importDeclarationMeta, isDefault = false, isSideEffects = false, singleQuotes = false) => {
    return (context) => {
        return (rootNode) => {
            var _a;
            ...
            importDeclarationMeta.identifiers = identifiers = identifiers.filter((identifier) => {
                return !importDeclarationCollides(identifier, sourceFile, importDeclarationMeta.moduleName, isSideEffects);
            });
            ...
            for (let i = 0; i < newStatements.length; i++) {
                ...
                if (ts.isImportDeclaration(statement) &&
                    ...
                    const namedBindings = (_a = statement.importClause) === null || _a === void 0 ? void 0 : _a.namedBindings;
                    if (namedBindings &&
                        ts.isNamedImports(namedBindings) &&
                        identifiers.length > 0) {
                        importDeclarationUpdated = true;
                        const updatedImportSpecifiers = [
                            ...namedBindings.elements,
                            ...identifiers.map(createImportSpecifierWithOptionalAlias),
                        ];
                        const updatedNamedImports = context.factory.updateNamedImports(namedBindings, updatedImportSpecifiers);
                        const updatedImportClause = context.factory.updateImportClause(statement.importClause, false, statement.importClause.name, updatedNamedImports);
                        newStatements[i] = context.factory.updateImportDeclaration(statement, statement.modifiers, updatedImportClause, statement.moduleSpecifier, statement.attributes);
                    }
            ...
            if (!importDeclarationUpdated && identifiers.length > 0) {
                const newImportDeclaration = createImportDeclaration(importDeclarationMeta, isDefault, isSideEffects, singleQuotes);
                newStatements = [
...
exports.newImportDeclarationTransformerFactory = newImportDeclarationTransformerFactory;
//#endregion
//#region Helpers
function createImportDeclaration(importDeclarationMeta, isDefault = false, isSideEffects = false, singleQuotes = false) {
    if (isSideEffects) {
        // create a side effects declaration of the form `import "my-module"`
        return ts.factory.createImportDeclaration(undefined, // modifiers
        undefined, // import clause
        ts.factory.createStringLiteral(importDeclarationMeta.moduleName, singleQuotes) // module specifier
        );
    }
    ...
    if (isDefault) {
        importClause = ts.factory.createImportClause(false, // is type only
        ts.factory.createIdentifier(identifiers[0].name), // name - import X from "module"
        undefined // named bindings
        );
    }
    else {
        const namedImport = ts.factory.createNamedImports(identifiers.map(createImportSpecifierWithOptionalAlias));
        importClause = ts.factory.createImportClause(false, // is type only
        undefined, // name
        namedImport // named bindings - import { X, Y... } from "module"
        );
    }
    const importDeclaration = ts.factory.createImportDeclaration(undefined, // modifiers
    importClause, ts.factory.createStringLiteral(importDeclarationMeta.moduleName, singleQuotes) // module specifier
    );
    return importDeclaration;
}
function importDeclarationCollides(identifier, sourceFile, moduleName, isSideEffects = false) {
    ...
    if (isSideEffects && moduleName) {
        return Array.from(allImportedIdentifiers.values()).some((importMeta) => importMeta.moduleName === moduleName);
    }
    const collidesWithExisting = Array.from(allImportedIdentifiers.values()).some((importMeta) => {
        let collides = false;
        ...
        if (identifier.alias) {
            return (identifier.alias === importMeta.identifiers.name ||
 ...
 */
function isPropertyAssignment(obj) {
    return (obj &&
        'name' in obj &&
        'value' in obj &&
        (ts.isExpression(obj.value) ||
            ts.isNumericLiteral(obj.value) ||
            ts.isStringLiteral(obj.value)));
}
/**
 ...
 */
function updatePropertyAssignmentWithIdentifier(newProperty, existingProperty, context, node) {
    ...
    const updatedProperty = ts.isPropertyAssignment(existingProperty)
        ? context.factory.updatePropertyAssignment(existingProperty, existingProperty.name, newPropInitializer)
        : context.factory.updateShorthandPropertyAssignment(existingProperty, existingProperty.name, newPropInitializer);
 ...
 */
function updatePropertyAssignmentWithArrayLiteral(newProperty, existingProperty, context, node, expressionCollector, multiline) {
    const existingPropInitializer = ts.isPropertyAssignment(existingProperty)
        ? existingProperty.initializer
        : existingProperty.objectAssignmentInitializer;
    const newPropInitializer = ts.isPropertyAssignment(newProperty)
    ...
    const initializerAsArray = existingPropInitializer;
    ...
    const valueExpression = context.factory.createArrayLiteralExpression(uniqueElements, multiline);
    const updatedProperty = ts.isPropertyAssignment(existingProperty)
        ? context.factory.updatePropertyAssignment(existingProperty, existingProperty.name, valueExpression)
        : context.factory.updateShorthandPropertyAssignment(existingProperty, existingProperty.name, valueExpression);
    const structure = Array.from(node.properties);
    const targetIndex = structure.indexOf(existingProperty);
    if (targetIndex > -1) {
        // attempt to modify the property assignment and preserve the order
 ...
 */
function createImportSpecifierWithOptionalAlias(identifier) {
    ...
    return ts.factory.createImportSpecifier(false, // is type only
    identifier.alias ? ts.factory.createIdentifier(identifier.name) : undefined, ts.factory.createIdentifier(aliasOrName));
}
/**
 ...
 */
function gatherImportDeclarations(statements) {
    const allImportedIdentifiers = new Map();
    let i = 0;
    for (const statement of statements) {
        ...
        if (!statement.importClause) {
            ...
            const sideEffectsName = `${util_1.SIDE_EFFECTS_IMPORT_TEMPLATE_NAME}_${++i}`;
